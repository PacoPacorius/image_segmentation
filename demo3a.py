...
def demo3a():
    """
    Demo 3a: Non-recursive normalized cuts on images d2a and d2b
    ...
    fig.suptitle('Demo 3a: Non-recursive Normalized Cuts on Images', fontsize=16)
    ...
    for img_idx, (img, img_name) in enumerate(zip(images, image_names)):
        ...
        # Perform normalized cuts for different k values
        for k_idx, k in enumerate(k_values):
            print(f"  Performing normalized cuts with k={k}...")
            
            # Perform normalized cuts
            cluster_labels = n_cuts(affinity_mat, k)
